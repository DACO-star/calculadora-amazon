...
# ==========================================
# CALCUAMZ v4.3.2 - FIX KEYERROR & FULL RESTORE
# ==========================================

st.set_page_config(layout="wide", page_title="CalcuAMZ v4.3.2", page_icon="📦")
...
def calcular_detallado(r):
    try:
        ...
        # Buscamos '% FEE' o 'FEE' por si acaso
        p_fee_pct = clean(r.get('% FEE', r.get('FEE', 4.0)))
        env = clean(r.get('ENVIO', 80.0))
        # Buscamos 'TIPO CAMBIO' o 'TC'
        t_c = clean(r.get('TIPO CAMBIO', r.get('TC', 18.00)))
...
if not st.session_state.auth:
    ...
    if not df_raw.empty:
        ...
        # ASEGURAR COLUMNAS MINIMAS PARA EVITAR KEYERROR
        for c in ['SKU', 'PRODUCTO', 'COSTO USD', 'AMAZON', 'ENVIO', '% FEE', 'TIPO CAMBIO']:
            if c not in df_raw.columns: df_raw[c] = 0 if c != 'PRODUCTO' else "SIN NOMBRE"
        ...
        st.title("📊 Dacocel Master Dashboard v4.3.2")
        ...
        with t2:
            ...
            sel = st.selectbox("Selecciona:", opcs)
            ...
            with st.form("f_edit"):
                ...
                ecos = ce1.number_input("Costo USD", value=clean(curr['COSTO USD']))
                epre = ce2.number_input("Precio AMZ", value=clean(curr['AMAZON']))
                eenv = ce3.number_input("Envío", value=clean(curr['ENVIO']))
                efee = ce4.number_input("% Fee", value=clean(curr['% FEE']))
                etc = ce5.number_input("T. Cambio", value=clean(curr['TIPO CAMBIO']))
        ...
        st.divider()
        # TABLA FINAL CON SEMÁFORO Y TODOS LOS DATOS
        ...
        busq = c_bus.text_input("🔍 Filtro...").upper()
